...
void sparse_sub(int* Ap, int* Bp, int* Cp, int* Ai, int* Bi, int* Ci, double* Ax, double* Bx, double* Cx, int Cr, int Cc);
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    int* Bp; /** pointer of matrix B (prhs[1]) */
    int* Cp; /** pointer of matrix C (plhs[0], return value) */
    int* Ai; /** index of matrix A (prhs[0]) */
    int* Bi; /** index of matrix B (prhs[1]) */
    int* Ci; /** index of matrix C (plhs[0], return value)*/
  ...
  /** check input-output numbers */
    ...
    /** Pointer of C matrix */
    Cp = (int*)mxMalloc((Cc+1)*sizeof(int)); 
    Ci = (int*)mxMalloc((Cnnz)*sizeof(int));
    Cx = (double*)mxMalloc((Cnnz)*sizeof(double));
    

    /** call the real function. */
    sparse_sub(Ap, Bp, Cp, Ai, Bi, Ci, Ax, Bx, Cx, Cr, Cc);
...
void sparse_sub(int* Ap, int* Bp, int* Cp, int* Ai, int* Bi, int* Ci, double* Ax, double* Bx, double* Cx, int Cr, int Cc)
{
   int i, j;
    ...
    /** flags */
  int* w = (int*) mxMalloc(Cr*sizeof(int));
    /** values */
